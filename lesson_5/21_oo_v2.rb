...
class Participant
  attr_accessor :name, :hand, :points, :score
  ...
  def possible_points
    ...
    point_combinations.map(&:sum).sort
  end
end

class Dealer < Participant
  DEALER_NAMES = %w(R2D2 WallE C3PO Hall9000 ChatGPT)
...
module Displays
  ...
  def display_table(show_dealer_cards=false)
    system 'clear'
    puts '========================='
    puts "BlackJack at #{blackjack_points}"
...
class BJGame
  ...
  attr_accessor :player, :dealer, :deck, :blackjack_points, :dealer_hit_max
  ...
  def take_dealer_turn
    ...
    loop do
      sleep(1)
      puts
      break if busted_or_blackjack?(dealer)
      if dealer_above_max?
        stay(dealer)
        break
      end
